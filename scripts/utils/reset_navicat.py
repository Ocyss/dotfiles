...
def check_network_connection_status(check_timeout_minutes=60, check_time_interval_second=2):
    ...
    while not status and network_check_count > 0:
        ...
        result = subprocess.run(
            'ping baidu.com -c 1', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def check_ntp_service_status(check_timeout_minutes=10):
    ...
    while not status and ntp_service_check_count > 0:
        ...
        result = subprocess.run('timedatectl status | grep NTP | cut -d ":" -f 2',
        ...
        if "inactive" in ntp_service_msg:
            ...
            subprocess.run('timedatectl set-ntp false', shell=True,
            ...
            # log.debug(str(outs, 'utf-8'))
            # log.debug(str(errs, 'utf-8'))
            subprocess.run('timedatectl set-ntp true', shell=True,
                           stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            sleep(4)
        else:
            status = True
...
def check_time_sync_status(check_timeout_minutes=60, check_time_interval_second=5):
    ...
    while not status and time_sync_check_count > 0:
        ...
        result = subprocess.run('timedatectl status | grep synchronized | cut -d ":" -f 2',
                                shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    if reset_json_file_exists:
        try:
            ...
            if count_days > 0 and count_days <= 14:
                log.info('试用期至 %s, 下次重置将在 %d 天后进行',
                         trial_period_str, count_days)
                return False
            elif count_days == 0:
...
def update_navicat_reset_json_data(reseted):
    ...
    trial_period_str = (now_date + datetime.timedelta(days=14)
                        ).strftime(DATE_FORMAT_PATTERN)
    if not reseted:
        json_data = get_json_data(RESET_JSON_INFO_FILE)
...
def reset_navicat():
    ...
    if need_to_reset_navicat():
        ...
        result = subprocess.run(
            DCONF_RESET_CMD, shell=True, capture_output=True, text=True)
...
if __name__ == '__main__':
    ...
    NAVICAT_PREFERENCES_JSON_PATH = USER_HOME_DIR + \
    ...
    if return_code:
        ...
        if sync_status:
            ...
            now_date = datetime.datetime.now()
            log.info('时间同步失败，使用本机系统时间：%s，该时间直接影响到 navicat 的试用期',
