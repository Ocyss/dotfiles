...
PrivateKEY = "8136380aabbb4ddf968ad7c34e72dd80"
...
def getWeatherNow():
    ...
    # print(res)
    now = res["now"]
    with open(path + "/weather.cache", mode="w", encoding="utf-8") as f:
        json.dump(now, f)
    return now
...
try:
    match sys.argv[1]:
        ...
        case "click":
            content = ""
            with open(path + "/weather.cache", "r", encoding="utf-8") as f:
                now = json.load(f)
                t = datetime.datetime.strptime(now['obsTime'], "%Y-%m-%dT%H:%M%z").replace(tzinfo=None)
