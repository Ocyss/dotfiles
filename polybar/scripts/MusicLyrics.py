    ...
    while True:
        try:
            ...
            while l<len(m) and m[l]['time'] < t:
                l += 1
            # s = "%{{F#9ece6a}}[%{{F-}}{0}%{{F#9ece6a}}]%{{F-}}  %{{F#8897f4}}%{{F-}} {1:<3}"
            # print(s.format(len_format(m[l-1]['lyric']),status['volume'],chr(12288)))
            lyrics = "%{{F#79E6F3}}%{{F-}} {0:<3} %{{F#9ece6a}}[%{{F-}}{1}%{{F#9ece6a}}]%{{F-}}".format(status['volume'],m[l-1]['lyric'])
